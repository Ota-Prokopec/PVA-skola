<script lang="ts">
    ...
    const usersPromise =  f("/getUsersInGroup.php", "POST", {groupId: params.groupId})
...
<div class="flex flex-wrap flex-col gap-4">
    <div class="text-4xl">Uživatelé ve skupině </div>
   ...
   <button on:click={() => navigate(`/user/update/${user.userId}`)} class="flex items-center space-x-4  border-2 hover:border-4 border-green-500 rounded-xl">
    <Avatar  rounded>{user.username.split(" ")[0][0]} {user.username.split(" ")[1][0]}</Avatar>
